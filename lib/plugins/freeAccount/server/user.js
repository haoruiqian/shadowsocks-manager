...
const allowedEmailDomains = config.plugins.freeAccount.allowedEmailDomains || [];
...
const sendEmail = (req, res) => {
  ...
  const emailDomain = addressParser.parseOneAddress(emailAddress).domain;
  if (allowedEmailDomains.length > 0 && !allowedEmailDomains.includes(emailDomain)) {
    // Email domain is not allowed, fail.
    return res.send('success'); // Don't tell the user :)
  }
  email.sendCode(emailAddress, 'Shadowsocks 验证码', '您的验证码是:').then(() => {
